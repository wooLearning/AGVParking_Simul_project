...
#endif
#define ORIENT_STATE_COUNT (NUM_DIRECTIONS + 1)
#define ROT_WAIT_STATE_COUNT ((TURN_90_WAIT) > 0 ? (TURN_90_WAIT) : 1)
 ...
 */
 // --- ST-A* 정적 버퍼 크기 ---
#define MAX_TOT (((MAX_WHCA_HORIZON)+1) * GRID_WIDTH * GRID_HEIGHT * ORIENT_STATE_COUNT * ROT_WAIT_STATE_COUNT)
...
static const char* path_algo_label(PathAlgo algo) {
    switch (algo) {
    case PATHALGO_ASTAR_SIMPLE: return "A* (단순)";
    case PATHALGO_DSTAR_BASIC:  return "D* Lite (기본)";
    case PATHALGO_DEFAULT:
    default: return "Default (WHCA*+D*Lite+WFG+CBS)";
    }
}
...
static inline int st_dir_to_index(AgentDir dir) { return (dir == DIR_NONE) ? 0 : ((int)dir + 1); }
static inline AgentDir st_index_to_dir(int idx) { return (idx <= 0) ? DIR_NONE : (AgentDir)(idx - 1); }
static inline int st_clamp_rot(int rot) { if (rot < 0) return 0; int maxv = ROT_WAIT_STATE_COUNT - 1; return (rot > maxv) ? maxv : rot; }

static inline void st_unpack_index(int idx, int width, int height, int* t, int* y, int* x, int* orient_idx, int* rot_idx) {
    int tmp = idx;
    int x_local = tmp % width; tmp /= width;
    int y_local = tmp % height; tmp /= height;
    int rot_local = tmp % ROT_WAIT_STATE_COUNT; tmp /= ROT_WAIT_STATE_COUNT;
    int orient_local = tmp % ORIENT_STATE_COUNT; tmp /= ORIENT_STATE_COUNT;
    if (x) *x = x_local;
    if (y) *y = y_local;
    if (rot_idx) *rot_idx = rot_local;
    if (orient_idx) *orient_idx = orient_local;
    if (t) *t = tmp;
}
...
static void simulation_display_status(const Simulation* sim, int is_paused) {
    ...
    // 알고리즘 표시
    {
        const char* algo = path_algo_label(sim->path_algo);
...
static int st_astar_plan_single(const Agent* agent, GridMap* map, Node* start, Node* goal, int horizon,
    int ext_occ[MAX_WHCA_HORIZON + 1][GRID_HEIGHT][GRID_WIDTH],
    const CBSConstraint* cons, int ncons,
    Node* out_plan[MAX_WHCA_HORIZON + 1]) {
    /**
     * @brief 시간-공간 A*를 사용하여 다중상시 노겨된 그룹 내 동안 하나의 에이전트에 대한 1개Åb8 계획을 수립합니다.
     *        각 상태는 위치, 시간, 허능한 고비모드(heading)와 회전 대기시간을 포함합니다.
     * @param agent 현재 에이전트 표지자가 함께 이동하는 구성체가 제공한 정보명령합니다.
     * @param map 지도 데이터류입니다.
     * @param start 시작 노드입니다.
     * @param goal 목표 노드(수닥 중)입니다.
     */
    if (!start) return 0;
    int agent_id = agent ? agent->id : -1;
    int T = horizon;
    int W = GRID_WIDTH, H = GRID_HEIGHT;
    int TOT = (T + 1) * W * H * ORIENT_STATE_COUNT * ROT_WAIT_STATE_COUNT;
...
#ifndef ST_INDEX
#define ST_INDEX(t,y,x,orient_idx,rot_idx,width,height) (((((t) * ORIENT_STATE_COUNT + (orient_idx)) * ROT_WAIT_STATE_COUNT + (rot_idx)) * (height) + (y)) * (width) + (x))
#endif

    int sx = start->x, sy = start->y;
    int gx = goal ? goal->x : sx, gy = goal ? goal->y : sy;
    int start_orient = st_dir_to_index(agent ? agent->heading : DIR_NONE);
    int start_rot = st_clamp_rot(agent ? agent->rotation_wait : 0);

    for (int t = 0; t <= T; t++) out_plan[t] = NULL;
    out_plan[0] = start;

    int start_idx = ST_INDEX(0, sy, sx, start_orient, start_rot, W, H);
    g[start_idx] = 0.0;
    ...
    while (1) {
        ...
        int ct, cx, cy, orient_idx, rot_idx;
        st_unpack_index(cur, W, H, &ct, &cy, &cx, &orient_idx, &rot_idx);
        AgentDir current_heading = st_index_to_dir(orient_idx);
        int rot_wait = rot_idx;

        if (goal && cx == gx && cy == gy) { best_idx = cur; break; }
        ...
        for (int k = 0; k < DIR5_COUNT; k++) {
            ...
            if (k == 0) {
                ...
                int new_rot = (rot_wait > 0) ? rot_wait - 1 : 0;
                int nid = ST_INDEX(nt, ny, nx, orient_idx, new_rot, W, H);
            ...
            AgentDir desired_heading = dir_from_delta(nx - cx, ny - cy);
            if (desired_heading == DIR_NONE) continue;

            if (rot_wait > 0) continue;

            int new_orient_idx = st_dir_to_index(desired_heading);

            if (current_heading != DIR_NONE) {
                int turn_steps = dir_turn_steps(current_heading, desired_heading);
                if (turn_steps == 1) {
                    if (ext_occ[nt][cy][cx] != -1) continue;
                    if (violates_constraint_for(agent_id, ct, cx, cy, cx, cy, cons, ncons)) continue;

                    int new_rot = st_clamp_rot((TURN_90_WAIT > 0) ? TURN_90_WAIT - 1 : 0);
                    int nid = ST_INDEX(nt, cy, cx, new_orient_idx, new_rot, W, H);
                    if (closed[nid]) continue;

                    double ng = g[cur] + 1.0;
                    if (ng + 1e-9 < g[nid]) {
                        g[nid] = ng;
                        double h = goal ? manhattan_xy(cx, cy, gx, gy) : 0.0;
                        f[nid] = ng + h;
                        prev[nid] = cur;
                        open[nid] = 1;
                    }
            ...
            int nid = ST_INDEX(nt, ny, nx, new_orient_idx, 0, W, H);
            if (closed[nid]) continue;
    ...
    int trace = best_idx;
    while (trace != -1 && plen < (MAX_WHCA_HORIZON + 1)) { path_idx[plen++] = trace; trace = prev[trace]; }
    if (plen == 0) {
    ...
    for (int t = 0; t < plen; t++) {
        ...
        int tt, xx, yy, oidx, ridx;
        st_unpack_index(idx, W, H, &tt, &yy, &xx, &oidx, &ridx);
        if (tt <= T) out_plan[tt] = &map->grid[yy][xx];
    }
    Node* last = out_plan[0];
    for (int t = 1; t <= T; t++) {
        if (!out_plan[t]) out_plan[t] = last;
        else last = out_plan[t];
    }
    return 1;
}
...
static int run_partial_CBS(AgentManager* m, GridMap* map, Logger* lg,
    ...
    for (int i = 0; i < group_n; i++) {
        ...
        if (!st_astar_plan_single(&m->agents[id], map, m->agents[id].pos, m->agents[id].goal, g_whca_horizon, ext_occ,
...
void simulation_print_performance_summary(const Simulation* sim) {
    ...
    printf(" Mode                                : %s\n", mode_label);
    printf(" Map ID                              : %d\n", sim->map_id);
    printf(" Path Algorithm                      : %s\n", path_algo_label(sim->path_algo));
    printf(" Total Physical Time Steps           : %d\n", recorded_steps);
    {
        int active_agents = 0;
        if (am) {
